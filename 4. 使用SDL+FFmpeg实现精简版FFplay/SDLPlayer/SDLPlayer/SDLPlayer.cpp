...
static int upload_texture(SDL_Texture **tex, AVFrame *frame, struct SwsContext **img_convert_ctx) {
	...
	case SDL_PIXELFORMAT_UNKNOWN:
		...
		*img_convert_ctx =
			sws_getCachedContext(*img_convert_ctx, frame->width, frame->height, (AVPixelFormat)frame->format, frame->width,
...
static int audio_thread(void *arg) {
	...
	do {
		...
		if (got_frame) {
			...
			AVRational ration;
			ration.num = frame->nb_samples;
			ration.den = frame->sample_rate;
			af->duration = av_q2d(ration);
...
static int video_thread(void *arg) {
	...
	for (;;) {
		...
		if (!ret) continue;
		AVRational ration;
		ration.num = frame_rate.den;
		ration.den = frame_rate.num;
